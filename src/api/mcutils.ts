...
export async function getMcutilsJarUrl(engine: string, version: string): Promise<ServerJarUrl> {
  return {
...
export async function getPaperJarUrl(version: string): Promise<ServerJarUrl> {
  ...
  if (!response.ok) {
    throw new Error(`Failed to fetch Paper builds: ${response.status} ${response.statusText}`);
  }

  const data = await response.json() as { builds: Array<{ build: number; channel: string; downloads: { application: { name: string } } }> };
...
export async function getFoliaJarUrl(version: string): Promise<ServerJarUrl> {
  ...
  const response = await fetch(apiUrl);
  if (!response.ok) {
    throw new Error(`Failed to fetch Folia builds: ${response.status} ${response.statusText}`);
  }
...
export async function getPurpurJarUrl(version: string): Promise<ServerJarUrl> {
  ...
  const response = await fetch(apiUrl);
  if (!response.ok) {
    throw new Error(`Failed to fetch Purpur builds: ${response.status} ${response.statusText}`);
  }

  const data = await response.json() as { builds: { all: string[]; latest: string } };
...
// NeoForge
export async function getNeoForgeJarUrl(version: string): Promise<ServerJarUrl> {
  return getMcutilsJarUrl('neoforge', version);
}

// Spigot
export async function getSpigotJarUrl(version: string): Promise<ServerJarUrl> {
  return getMcutilsJarUrl('spigot', version);
}

// Main function to get jar URL based on engine
export async function getServerJarUrl(engine: string, version: string): Promise<ServerJarUrl> {
...
export function getCompatibleEngines(version: string): Array<{ name: string; description: string }> {
  const engines = [
    { name: 'Vanilla', description: 'Oficjalny serwer Mojang - wszystkie wersje obsługiwane' },
  ];
  ...
  if (v.major > 1 || (v.major === 1 && v.minor >= 8)) {
    if (v.minor > 8 || (v.minor === 8 && v.patch >= 8)) {
      engines.push({ name: 'Paper', description: 'Wydajny fork Spigota z optymalizacjami' });
    }
  }
  ...
  if (v.major > 1 || (v.major === 1 && v.minor >= 8)) {
    if (v.minor > 8 || (v.minor === 8 && v.patch >= 8)) {
      engines.push({ name: 'Spigot', description: 'Popularny fork Bukkit z ulepszeniami wydajności' });
    }
  }
  ...
  if (v.major > 1 || (v.major === 1 && v.minor >= 14)) {
    if (v.minor > 14 || (v.minor === 14 && v.patch >= 4)) {
      engines.push({ name: 'Purpur', description: 'Fork Paper z dodatkowymi opcjami konfiguracji' });
    }
  }
  ...
  if (v.major > 1 || (v.major === 1 && v.minor >= 19)) {
    if (v.minor > 19 || (v.minor === 19 && v.patch >= 4)) {
      engines.push({ name: 'Folia', description: 'Wielowątkowy fork Paper dla dużych serwerów' });
    }
  }
  ...
  engines.push({ name: 'Forge', description: 'Popularne API do modowania Minecraft' });
  
  // NeoForge - 1.20.1+
  if (v.major > 1 || (v.major === 1 && v.minor >= 20)) {
    engines.push({ name: 'NeoForge', description: 'Nowoczesny fork Forge dla nowszych wersji' });
  }
